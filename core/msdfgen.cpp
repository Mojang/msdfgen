...
struct MultiDistance {
    double r, g, b;
    double med;
	double sdf; // Some forms use this extra field
};
...
// -------------------------------------------------------------------------------------------------
// Added by Mikaus Whiteley.  It is basically the same as the above, but works on the fourth element
...
void msdfErrorCorrection(Bitmap<FloatRGBA> &output, const Vector2 &threshold) {
    ...
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
			FloatRGBA& pixel = output(x, y);
			if ((x > 0 && pixelClash(pixel, output(x - 1, y), threshold.x))
				|| (x < w - 1 && pixelClash(pixel, output(x + 1, y), threshold.x))
				|| (y > 0 && pixelClash(pixel, output(x, y - 1), threshold.y))
				|| (y < h - 1 && pixelClash(pixel, output(x, y + 1), threshold.y))) {

				// If there is a pixel clash set the value to the median of all the different edges
				float med = median(pixel.r, pixel.g, pixel.b);
				pixel.r = med, pixel.g = med, pixel.b = med;
			}
        }
...
#endif
    {
        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                double negDist = -SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                double posDist = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                int winding = 0;
                ...
                double sd = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
...
#endif
    {
        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                double psd = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                if (posDist >= 0 && fabs(posDist) <= fabs(negDist)) {
                    psd = posDist;
                    winding = 1;
                    for (int i = 0; i < contourCount; ++i)
...
#endif
    {
        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                double d = fabs(SignedDistance::INFINITE_SIGNED_DISTANCE.distance);
                double negDist = -SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                double posDist = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                int winding = 0;
                ...
                MultiDistance msd;
                msd.r = msd.g = msd.b = msd.med = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                if (posDist >= 0 && fabs(posDist) <= fabs(negDist)) {
                    msd.med = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
                    winding = 1;
                    for (int i = 0; i < contourCount; ++i)
...
// ----------------------------------------------------------------------------------------------
// Combining both SDF and MSDF in order to condense looping through the contours multiple times
// Added by Mikaus Whiteley

	// This is a combination of both the MSDF and SDF functions.  This is an optimization to prevent us from having to go over all of the contours multiple times.
	void generateMSDFAndSDF(Bitmap<FloatRGBA> &output, const Shape &shape, double range, const Vector2 &scale, const Vector2 &translate, double edgeThreshold) {
		int contourCount = shape.contours.size();
		int w = output.width(), h = output.height();
		std::vector<int> windings;
		std::vector<MultiDistance> contourSD;
		...
		for (int y = 0; y < h; ++y) {
			for (int x = 0; x < w; ++x) {
				...
				double negDist = -SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
				double posDist = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
				double sdfNegDist = -SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
				double sdfPosDist = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
				Point2 centerPoint = Vector2(x + .5, y + .5) / scale - translate;
				...
				for (int i = 0; i < contourCount; ++i, ++contour) {
					...
					r.nearEdge = g.nearEdge = b.nearEdge = NULL;
					r.nearParam = g.nearParam = b.nearParam = 0;
					SignedDistance sdfMinDistance;

					for (std::vector<EdgeHolder>::const_iterator edge = contour->edges.begin(); edge != contour->edges.end(); ++edge) {
						double param;
						SignedDistance distance = (*edge)->signedDistance(centerPoint, param);
						if ((*edge)->color&RED && distance < r.minDistance) {
							r.minDistance = distance;
							r.nearEdge = &*edge;
							r.nearParam = param;
						...
						if (distance < sdfMinDistance) {
							sdfMinDistance = distance;
						}
					}
					...
					if (r.nearEdge) {
						(*r.nearEdge)->distanceToPseudoDistance(r.minDistance, centerPoint, r.nearParam);
					}
					if (g.nearEdge) {
						(*g.nearEdge)->distanceToPseudoDistance(g.minDistance, centerPoint, g.nearParam);
					}
					if (b.nearEdge) {
						(*b.nearEdge)->distanceToPseudoDistance(b.minDistance, centerPoint, b.nearParam);
					}
					...
					contourSD[i].sdf = sdfMinDistance.distance;
					if (windings[i] > 0 && sdfMinDistance.distance >= 0 && fabs(sdfMinDistance.distance) < fabs(sdfPosDist))
						sdfPosDist = sdfMinDistance.distance;
					if (windings[i] < 0 && sdfMinDistance.distance <= 0 && fabs(sdfMinDistance.distance) < fabs(sdfNegDist))
						sdfNegDist = sdfMinDistance.distance;
				}
				if (sr.nearEdge) {
					(*sr.nearEdge)->distanceToPseudoDistance(sr.minDistance, centerPoint, sr.nearParam);
				}
				if (sg.nearEdge) {
					(*sg.nearEdge)->distanceToPseudoDistance(sg.minDistance, centerPoint, sg.nearParam);
				}
				if (sb.nearEdge) {
					(*sb.nearEdge)->distanceToPseudoDistance(sb.minDistance, centerPoint, sb.nearParam);
				}
				...
				msdfgen::FloatRGBA& pixel = output(x, y);
				pixel.r = float(msd.r / range + .5);
				pixel.g = float(msd.g / range + .5);
				pixel.b = float(msd.b / range + .5);

				// SDF
				double sd = SignedDistance::INFINITE_SIGNED_DISTANCE.distance;
				if (sdfPosDist >= 0 && fabs(sdfPosDist) <= fabs(sdfNegDist)) {
					sd = sdfPosDist;
					winding = 1;
					for (int i = 0; i < contourCount; ++i)
						if (windings[i] > 0 && contourSD[i].sdf > sd && fabs(contourSD[i].sdf) < fabs(sdfNegDist))
							sd = contourSD[i].sdf;
				}
				else if (sdfNegDist <= 0 && fabs(sdfNegDist) <= fabs(sdfPosDist)) {
					sd = sdfNegDist;
					winding = -1;
					for (int i = 0; i < contourCount; ++i)
						if (windings[i] < 0 && contourSD[i].sdf < sd && fabs(contourSD[i].sdf) < fabs(sdfPosDist))
							sd = contourSD[i].sdf;
				}
				for (int i = 0; i < contourCount; ++i)
					if (windings[i] != winding && fabs(contourSD[i].sdf) < fabs(sd))
						sd = contourSD[i].sdf;

				pixel.a = float(sd / range + .5);
			}
		}
